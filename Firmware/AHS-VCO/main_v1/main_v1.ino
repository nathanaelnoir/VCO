// Optimized reference sketch.
// Kept disabled because every .ino in this folder is compiled into one Arduino sketch.
#if 0
#include <MozziGuts.h>
...
float freq1 = 110.0f;//base freq of OSC1
float freqv1 = 440.0f;// freq1 apply voct
uint16_t freq1_raw = 880;
uint16_t freq1_raw_old = 0xffff;
uint16_t voct = 1000;//external V/OCT LSB
uint16_t voct_old = 0xffff;
byte harm_knob = 0;//AD wave knob
byte harm_knob_old = 0xff;//AD wave knob
byte gain = 127;
byte gain_old = 0xff;
...
byte gain_cache[7];
#endif
const byte gain_table[8][256] PROGMEM = {
...
const byte harm_table[8][256] PROGMEM = {
...
#if 0
void refreshGainCache() {
 gain_cache[0] = pgm_read_byte(&(gain_table[0][gain]));
 gain_cache[1] = pgm_read_byte(&(gain_table[1][gain]));
 gain_cache[2] = pgm_read_byte(&(gain_table[2][gain]));
 gain_cache[3] = pgm_read_byte(&(gain_table[3][gain]));
 gain_cache[4] = pgm_read_byte(&(gain_table[4][gain]));
 gain_cache[5] = pgm_read_byte(&(gain_table[5][gain]));
 gain_cache[6] = pgm_read_byte(&(gain_table[6][gain]));
 gain_old = gain;
}

void refreshOscFrequencies() {
 const float voct_scale = pow(2.0f, voct * kVoctExponentStep);
 freqv1 = freq1 * voct_scale; // V/oct apply
...
{
 refreshGainCache();
 refreshOscFrequencies();
 startMozzi(CONTROL_RATE); // :)
}
void updateControl() {
 //harmonics
 harm_knob = Harm_Smooth.next((mozziAnalogRead(3) + mozziAnalogRead(5)) >> 3);

 //harmonics_gain
 gain = Gain_Smooth.next((mozziAnalogRead(1) + mozziAnalogRead(4)) >> 3);
 if (gain != gain_old) {
   refreshGainCache();
 }

 //OSC frequency knob
 freq1_raw = mozziAnalogRead(0);
 freq1 = freq1_raw * 0.125f;

 //frequency setting
 voct = mozziAnalogRead(7) ;
 if ((voct != voct_old) || (freq1_raw != freq1_raw_old) || (harm_knob != harm_knob_old))
 {
   refreshOscFrequencies();
   voct_old=voct; 
   freq1_raw_old=freq1_raw;
   harm_knob_old=harm_knob;
 }
}
int updateAudio() {
 return MonoOutput::from8Bit(aSin1.next() * gain_cache[0] / factor + aSin2.next() * gain_cache[1] / factor + aSin3.next() * gain_cache[2] / factor + aSin4.next() * gain_cache[3] / factor + aSin5.next() * gain_cache[4] / factor + aSin6.next() * gain_cache[5] / factor + aSin7.next() * gain_cache[6] / factor /*+ aSin8.next() * cached gain*/);
}
void loop() {
