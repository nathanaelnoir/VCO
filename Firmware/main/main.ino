/*
  AHS VCO firmware, Mozzi v2.0.4 / Arduino Nano

  This sketch keeps the original three-mode voice architecture:
    - FM voice
    - chord oscillator bank
    - additive sine bank

  Mozzi v2 reads audio configuration from these inline defines before
  <Mozzi.h>, so the library's MozziConfig.h file must stay untouched.
*/
...
constexpr byte kModeCvInput = 2;
constexpr byte kParam2MainInput = 3;
constexpr byte kParam1CvInput = 4;
constexpr byte kParam2CvInput = 5;
constexpr byte kLevelCvInput = 6;
constexpr byte kVoctInput = 7;

constexpr int kAdcMax = 1023;
constexpr int kAudio8Min = -128;
...
Oscil <SIN2048_NUM_CELLS, MOZZI_AUDIO_RATE> aSin7(SIN2048_DATA);
Oscil <SIN2048_NUM_CELLS, MOZZI_AUDIO_RATE> aSin8(SIN2048_DATA);
...
Oscil <SAW2048_NUM_CELLS, MOZZI_AUDIO_RATE> aSaw5(SAW2048_DATA);

Oscil <SQUARE_NO_ALIAS512_NUM_CELLS, MOZZI_AUDIO_RATE> aSqu1(SQUARE_NO_ALIAS512_DATA);
Oscil <SQUARE_NO_ALIAS512_NUM_CELLS, MOZZI_AUDIO_RATE> aSqu2(SQUARE_NO_ALIAS512_DATA);
Oscil <SQUARE_NO_ALIAS512_NUM_CELLS, MOZZI_AUDIO_RATE> aSqu3(SQUARE_NO_ALIAS512_DATA);
Oscil <SQUARE_NO_ALIAS512_NUM_CELLS, MOZZI_AUDIO_RATE> aSqu4(SQUARE_NO_ALIAS512_DATA);
Oscil <SQUARE_NO_ALIAS512_NUM_CELLS, MOZZI_AUDIO_RATE> aSqu5(SQUARE_NO_ALIAS512_DATA);

Oscil <TRIANGLE_HERMES_2048_NUM_CELLS, MOZZI_AUDIO_RATE> aTri1(TRIANGLE_HERMES_2048_DATA);
Oscil <TRIANGLE_HERMES_2048_NUM_CELLS, MOZZI_AUDIO_RATE> aTri2(TRIANGLE_HERMES_2048_DATA);
Oscil <TRIANGLE_HERMES_2048_NUM_CELLS, MOZZI_AUDIO_RATE> aTri3(TRIANGLE_HERMES_2048_DATA);
Oscil <TRIANGLE_HERMES_2048_NUM_CELLS, MOZZI_AUDIO_RATE> aTri4(TRIANGLE_HERMES_2048_DATA);
Oscil <TRIANGLE_HERMES_2048_NUM_CELLS, MOZZI_AUDIO_RATE> aTri5(TRIANGLE_HERMES_2048_DATA);

Oscil <CHEBYSHEV_3RD_256_NUM_CELLS, MOZZI_AUDIO_RATE> aChb1(CHEBYSHEV_3RD_256_DATA);
Oscil <CHEBYSHEV_3RD_256_NUM_CELLS, MOZZI_AUDIO_RATE> aChb2(CHEBYSHEV_3RD_256_DATA);
Oscil <CHEBYSHEV_3RD_256_NUM_CELLS, MOZZI_AUDIO_RATE> aChb3(CHEBYSHEV_3RD_256_DATA);
Oscil <CHEBYSHEV_3RD_256_NUM_CELLS, MOZZI_AUDIO_RATE> aChb4(CHEBYSHEV_3RD_256_DATA);
Oscil <CHEBYSHEV_3RD_256_NUM_CELLS, MOZZI_AUDIO_RATE> aChb5(CHEBYSHEV_3RD_256_DATA);

Oscil <HALFSIN256_NUM_CELLS, MOZZI_AUDIO_RATE> ahSin1(HALFSIN256_DATA);
Oscil <HALFSIN256_NUM_CELLS, MOZZI_AUDIO_RATE> ahSin2(HALFSIN256_DATA);
Oscil <HALFSIN256_NUM_CELLS, MOZZI_AUDIO_RATE> ahSin3(HALFSIN256_DATA);
Oscil <HALFSIN256_NUM_CELLS, MOZZI_AUDIO_RATE> ahSin4(HALFSIN256_DATA);
Oscil <HALFSIN256_NUM_CELLS, MOZZI_AUDIO_RATE> ahSin5(HALFSIN256_DATA);

Oscil <WAVESHAPE_SIGMOID_NUM_CELLS, MOZZI_AUDIO_RATE> aSig1(WAVESHAPE_SIGMOID_DATA);
Oscil <WAVESHAPE_SIGMOID_NUM_CELLS, MOZZI_AUDIO_RATE> aSig2(WAVESHAPE_SIGMOID_DATA);
Oscil <WAVESHAPE_SIGMOID_NUM_CELLS, MOZZI_AUDIO_RATE> aSig3(WAVESHAPE_SIGMOID_DATA);
Oscil <WAVESHAPE_SIGMOID_NUM_CELLS, MOZZI_AUDIO_RATE> aSig4(WAVESHAPE_SIGMOID_DATA);
Oscil <WAVESHAPE_SIGMOID_NUM_CELLS, MOZZI_AUDIO_RATE> aSig5(WAVESHAPE_SIGMOID_DATA);

Oscil <PHASOR256_NUM_CELLS, MOZZI_AUDIO_RATE> aPha1(PHASOR256_DATA);
Oscil <PHASOR256_NUM_CELLS, MOZZI_AUDIO_RATE> aPha2(PHASOR256_DATA);
Oscil <PHASOR256_NUM_CELLS, MOZZI_AUDIO_RATE> aPha3(PHASOR256_DATA);
Oscil <PHASOR256_NUM_CELLS, MOZZI_AUDIO_RATE> aPha4(PHASOR256_DATA);
Oscil <PHASOR256_NUM_CELLS, MOZZI_AUDIO_RATE> aPha5(PHASOR256_DATA);





// These names mirror the original sketch so the musical behavior is easy to
// compare while debugging against the older file.
int freq1 = 110;
int voct = 1000;
int voct2 = 1000;
...
static inline float readVoctPow(int index) {
 index = constrain(index, 0, kAdcMax);
 return pgm_read_float(&(voctpow[index]));
}
...
void setFmFrequencies(Q16n16 freq) {
 ...
 // FM intentionally reads the raw pair of main/CV controls here so modulation
 // reacts at the control-rate boundary, not at the cached voice-selection path.
 mod_freq = ((carrier_freq >> 8)  * (mozziAnalogRead(kParam1MainInput) / 2 +mozziAnalogRead(kParam1CvInput) / 2));

 // Keep the original scaling: the shift avoids overflow in the fixed-point
 // multiply while preserving the patch's original FM depth response.
 deviation = ((mod_freq >> 16) * (1 + mozziAnalogRead(kParam2MainInput)+mozziAnalogRead(kParam2CvInput)));
...
void updateControl() {
  ...
  Mode_CV = map(mozziAnalogRead(kModeCvInput),0,kAdcMax,0,kModeAdditive);
  Gain_CV_0 = map(mozziAnalogRead(kLevelCvInput),0,kAdcMax,0,255);
  Gain_CV_1 = Gain_CV_0;
  Gain_CV_2 = Gain_CV_0;

  AnalogRead0 = mozziAnalogRead(kPitchPotInput);
  AnalogRead1 = mozziAnalogRead(kParam1MainInput);
  AnalogRead3 = mozziAnalogRead(kParam2MainInput);
  AnalogRead4 = mozziAnalogRead(kParam1CvInput);
...
if (Mode == kModeAdditive) {
   ...
   voct = AnalogRead7 ; 
 // The additive bank uses one knob pair for harmonic spacing and the other for
 // per-partial gain balance, keeping pitch control independent.
 ...
 digitalWrite(kChordLedPin,LOW);

 }

 else if (Mode == kModeFm) {
  
 
  digitalWrite(kFmLedPin,HIGH);
  digitalWrite(kAdditiveLedPin,LOW);
  digitalWrite(kChordLedPin,LOW);

   voct = AnalogRead7;
 knob_freq = AnalogRead0;
 setFmFrequencies((2270658 + knob_freq * 5000) * pow(2, readVoctPow(voct))) ;

 }

  else {
  
  digitalWrite(kChordLedPin,HIGH);
  digitalWrite(kFmLedPin,LOW);
  digitalWrite(kAdditiveLedPin,LOW);

 // Chord mode uses the same musical interval table as the original sketch;
 // CV sums with the panel control before table lookup.
 chord = (AnalogRead3 / 128) + (AnalogRead5 / 128);
 chord = constrain(chord, 0, 7);

 // Maxing the inversion knob temporarily repurposes the chord selector as a
 // waveform selector, so stored chord and inversion values are not disturbed.
 inv_knob = AnalogRead1;
 inv = (inv_knob  / 64)+ (AnalogRead4 / 64);
 inv = constrain(inv, 0, 15);

 if (inv_knob < kWaveSelectThreshold) {
   switch (inv) {
     case 0:
       inv_aply1 = 0;
       inv_aply2 = 0;
 ...
 voct2 = AnalogRead7 ;
 freqv1 = freq1 * pow(2, readVoctPow(voct2 + kOctaveTableStep * inv_aply1 + note1));
 freqv2 = freq1 * pow(2, readVoctPow(voct2 + kOctaveTableStep * inv_aply2 + note2));
 freqv3 = freq1 * pow(2, readVoctPow(voct2 + kOctaveTableStep * inv_aply3 + note3));
 freqv4 = freq1 * pow(2, readVoctPow(voct2 + kOctaveTableStep * inv_aply4 + note4));
 freqv5 = freq1 * pow(2, readVoctPow(voct2 + note5));
...
if (Mode == kModeAdditive) {
 ...
 return MonoOutput::from8Bit(clipAudio8(aCarrier.phMod(modulation)*Gain_CV_1/256));
 ...
 switch (wave) {
   ...
   case kWaveSaw:
     return MonoOutput::from8Bit(clipAudio8((aSaw1.next() / 8 + aSaw2.next() / 8 + aSaw3.next() / 8 + aSaw4.next() / 8 + aSaw5.next() / 8 * inv_aply5)*Gain_CV_0/256));
     break;

   case kWaveSquare:
     return MonoOutput::from8Bit(clipAudio8((aSqu1.next() / 8 + aSqu2.next() / 8 + aSqu3.next() / 8 + aSqu4.next() / 8 + aSqu5.next() / 8 * inv_aply5)*Gain_CV_0/256));
     break;

   case kWaveTriangle:
     return MonoOutput::from8Bit(clipAudio8((aTri1.next() / 8 + aTri2.next() / 8 + aTri3.next() / 8 + aTri4.next() / 8 + aTri5.next() / 8 * inv_aply5)*Gain_CV_0/256));
     break;

   case kWaveSine:
     return MonoOutput::from8Bit(clipAudio8((aSin1.next() / 8 + aSin2.next() / 8 + aSin3.next() / 8 + aSin4.next() / 8 + aSin5.next() / 8 * inv_aply5)*Gain_CV_0/256));
     break;

   case kWaveChebyshev:
     return MonoOutput::from8Bit(clipAudio8((aChb1.next() / 8 + aChb2.next() / 8 + aChb3.next() / 8 + aChb4.next() / 8 + aChb5.next() / 8 * inv_aply5)*Gain_CV_0/256));
     break;

   case kWaveHalfSine:
     return MonoOutput::from8Bit(clipAudio8((ahSin1.next() / 8 + ahSin2.next() / 8 + ahSin3.next() / 8 + ahSin4.next() / 8 + ahSin5.next() / 8 * inv_aply5)*Gain_CV_0/256));
     break;

   case kWaveSigmoid:
     return MonoOutput::from8Bit(clipAudio8((aSig1.next() / 8 + aSig2.next() / 8 + aSig3.next() / 8 + aSig4.next() / 8 + aSig5.next() / 8 * inv_aply5)*Gain_CV_0/256));
     break;

   case kWavePhasor:
     return MonoOutput::from8Bit(clipAudio8((aPha1.next() / 8 + aPha2.next() / 8 + aPha3.next() / 8 + aPha4.next() / 8 + aPha5.next() / 8 * inv_aply5)*Gain_CV_0/256));
     break;
